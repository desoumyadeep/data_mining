...
class Cluster_Validation:
    ...
    def run_NMI(self):
        ...
        for key, value in self.temp_dict.items():
            ...
            p_c_i = self.predicted_dict[_pl_idx] / self.no_of_data_point if self.predicted_dict[_pl_idx] !=0 else 1
            p_t_j = self.ground_truth_dict[_gt_idx] / self.no_of_data_point if self.ground_truth_dict[_gt_idx] !=0 else 1
        ...
        _entropy_p = _entropy_p if _entropy_p!=0 else 1
        _entropy_gt = _entropy_gt if _entropy_gt!= 0 else 1
    ...
    def run_jaccard(self):
        ...
        return 0 if (_true_positive + _false_negative + _false_positive) == 0 else round(_true_positive / (_true_positive + _false_negative + _false_positive), 3)
