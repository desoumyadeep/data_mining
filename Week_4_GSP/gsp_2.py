...
class Gsp:
    ...
    def getMinSupportItemlistk(self, itemset) -> set:
        _itemset = []
        ...
        for key, value in _l_transaction_list.items():
            if (float(value) / self.rec_cnt) > self.min_support:
                self.pattern_dict[key] = value
                _itemset.append(key)
        return _itemset

    def joinSet(self, itemset):
    ...
    def run_algorithm(self):
        ...
        while len(_freqset) != 0:
            k += 1
            _itemset = self.joinSet(_freqset)
            _freqset = self.getMinSupportItemlistk(_itemset)
            print("Pass: {0} \tPattern Cnt : {1}".format(
                str(k), str(len(self.pattern_dict))))
        self.write_ref_file()
...
def main():
    ...
    ap = Gsp("/Users/soumyadeepde/OneDrive/Documents/Study/MS/202101 - Spring/CS_412_Coursework/Week_4_GSP/dataset/input/review_sample.txt",
                 "/Users/soumyadeepde/OneDrive/Documents/Study/MS/202101 - Spring/CS_412_Coursework/Week_4_GSP/dataset/output/review_sample_gsp_2.txt", 0.01)
    ap.run_algorithm()
    _end_time = time.time()
