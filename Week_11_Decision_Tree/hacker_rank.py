...
class Classifier:
    ...
    def split_dataset(self, sample_dataset, sample_label):
        ...
        if no_of_items > 1 or len(set(sample_label)) > 1:
            ...
            for i in range(no_of_features):
                ...
                split_pt_list = sorted(list(set(
                    (sorted_feature_list[j][0] + sorted_feature_list[j+1][0])/2 for j in range(len(sorted_feature_list)-1))))
    ...
    def grow_tree(self, sample_dataset, sample_label, depth=0):
        ...
        node = Node(_lbl_cntr)
        if depth < self.max_depth and len(sample_label) > 0 :
